...
Loader.prototype.get = function(options, callback) {
    ...
    if (process.env.EDGEMICRO_LOCAL) {
        ...
        analytics.source = "microgateway";
        analytics.proxy = "dummy";
        analytics.proxy_revision = "1";
        analytics.compress = false;
        analytics.key = options.keys.key;
        analytics.secret = options.keys.secret;
        //create config cache
        const cache = {
            edgemicro: config.edgemicro,
