...
function removeNullTarget(proxy) {
    ...
    async.parallel([
        ...
        try {
            ...
            if (proxyPattern) {
                ...
                //get new apiProxies length
                arrLen = apiProxies.length;
            }

            //cleanup null targets
            for (i = 0; i < arrLen; i++) {
                if (apiProxies[i].targetEndpoint.url == "null") {
                    debug("ignoring " + apiProxies[i].apiProxyName + " proxy since it has a null target");
                    delete apiProxies[i];
                }
            }
            //clean up null
