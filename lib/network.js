...
var agent_config_validator = require('./agent-validator');
var config_validator = require('./validator');
var debug = debug_('agent:config');
var ioLib = require('./io');

var retry;
var Loader = function (io) {
  this.io = io || ioLib();
};
...
Loader.prototype.get = function (options, callback) {
  ...
  var err = _validateUrls(this.config);
  if(err){
    return callback(err);
  }

  var config = _setDefaults(this.config);

  target = target || _setTarget(config);

  // initiate an immediate load, and setup retries if it fails
  _load(config, keys, function (err, proxies, products, hash) {
    if (proxies && products && hash != 0) {
      _mergeAndReturnConfig(config, keys, proxies, products, {io: io, target: target, hash: hash}, callback);
      _runInterval(config, keys, {
        io: io,
        target: target,
        interval: config.edge_config.refresh_interval || 0,
        intervalMin: 3600000,
        hash: hash
      })
    } else {
      ...
      _runInterval(config, keys, {
        io: io,
        target: target,
        interval: config.edge_config.retry_interval || 0,
        intervalMin: 5000,
        hash: hash
      })
      // start with the cached copy while we retry updates in the background
      var mergedConfig = io.loadSync({source:source});
      if (mergedConfig) {
...
/**
 * reload the config while constant polling
 * @param mergedConfig
 * @param keys
 * @param target
 * @param io
 ...
 * @private
 */
var _runInterval = function (config, keys, options) {
  if (retry) {
  ...
  if (options.interval > options.intervalMin) {
    retry = setInterval(function () {
      _load(config, keys, function (err, proxies, products, hash) {
        if (err) {
          return;
        }
        if (proxies && products && hash !== config._hash) {
          try {
            var fn = function () {
              console.log('iteration of timer')
            };
            _mergeAndReturnConfig(config, keys, proxies, products, {
              io: options.io,
...
var _load = function (config, keys, callback) {
  var options = {};
 ...
 * @private
 */
var _loadStatus = function (message, url, err, response, body, cb) {
 ...
 * @private
 */
var _merge = function (config, proxies, products, hash) {
...
var _mergeAndReturnConfig = function (config, keys, proxies, products, options, cb) {
  assert(options.io);
  assert(options.target);
  assert(options.hash);
  var mergedConfig = _merge(config, _mapEdgeProxies(proxies), _mapEdgeProducts(products), options.hash);
  _saveAndReturnConfig(mergedConfig, keys, options, cb);
};

var _saveAndReturnConfig = function (mergedConfig, keys, options, cb) {
  assert(options.target);
  assert(options.io);
...
var _mapEdgeProxies = function (proxies) {
  var mappedProxies = [];
...
var _mapEdgeProducts = function (products) {
  //const path_to_proxy = {};
...
var _validateTarget = function (target) {
...
var _validateUrls = function (config) {
