...
const _load = function(config, keys, callback) {
    ...
    async.parallel([
        ...
        if (!proxies) {
            console.error('warning:','no edge micro proxies found in response');
        }
        if (proxies.length === 0) {
        ...
        if (!productInfo) {
            console.error('warning:', 'no edge micro products found in response');
        }
        ...
        //Add public_key to custom plugin with name defined in custom_auth
        var customAuth = null;
        if(config.edgemicro && config.edgemicro.plugins && config.edgemicro.plugins.custom_auth) {
            customAuth = config.edgemicro.plugins.custom_auth;
        }
        ...
        if (!config.oauthv2)
            config.oauthv2 = {};
        if(customAuth && !config[customAuth]) {
            config[customAuth] = {};
        }
        if (results.length > 1 && results[2]) {
            ...
            if(customAuth) {
                config[customAuth].public_key = results[2];
            }
        } else {
        ...
        try {
            if (results.length > 1 && results[3]) {
                ...
                if(customAuth) {
                    config[customAuth].public_key = results[3];
                }
            }
        } catch (err) {
...
const _merge = function(config, proxies, products) {
    ...
    // copy properties to custom plugin with name defined in custom_auth
    var customAuth = null;
    if(updates.edgemicro.plugins && updates.edgemicro.plugins.custom_auth) {
        customAuth = config.edgemicro.plugins.custom_auth;
    }

    if(customAuth) {
        if (!updates[customAuth]) {
            updates[customAuth] = {};
        }
        updates[customAuth].path_to_proxy = products.path_to_proxy;
        updates[customAuth].product_to_proxy = products.product_to_proxy;
        updates[customAuth].product_to_api_resource = products.product_to_api_resource;
    }
