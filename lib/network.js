...
const _load = function (config, keys, callback) {
  ...
  async.parallel([
    ...
    if (results.length > 1 && results[2]) {
      ...
      _validateKey(results[2], (err, expired) => {
        if(err) {
          return callback(err);
        }

        if(expired) {
          console.error('warning:', 'jwt public key past expiration.');
        }

        return callback(null, proxies, products);
      });
    } else {
...
const _validateKey = function(key, cb) {
  pem.readCertificateInfo(key, (err, info) => {
    if(err) {
      return cb(err);
    }

    var d1 = new Date(info.validity.end);
    var d2 = new Date();
    return cb(null, d2 < d1);
  }); 
}
