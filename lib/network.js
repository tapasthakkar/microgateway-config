  ...
  _load(config, keys, function (err, proxies, products) {
    ...
    if (proxies && products) {
      ...
      if(config.edgemicro.proxies) {
        ...
        var filteredProxies = (config.edgemicro.proxies || []).map(function (proxy) {
          if (typeof(proxy) == "string") {
              return proxy; //this case, proxy is only listed by name, no associated subconfig overides
          } else {
            return Object.keys(proxy)[0];
          }
        });
