...
const _load = function(config, keys, callback) {
    ...
    async.parallel([
        function(cb) {
            ...
            if (process.env.EDGEMICRO_LOCAL_PROXY === "1") {
            ...
            } else if ( config.edge_config.redisBasedConfigCache === true ) { //retrieve info from redis db
                ...
                request.get(opts, function(err, response, body) {
                    ...
                    if(useSynchronizer && !err && response && response.statusCode === 200 && validateJSON(body)){ 
                        loader.saveConfigToRedis(config.edgemicro, globalOptions, config.edge_config.bootstrap, body, 'config', cb);
                    }
                });
            }
        },
        function(cb) {
             if ( config.edge_config.redisBasedConfigCache === true ) { //retrieve info from redis db
                ...
                request.get(opts, function(err, response, body) {
                    ...
                    if(useSynchronizer && !err && response && response.statusCode === 200 && validateJSON(body)){
                        loader.saveConfigToRedis(config.edgemicro, globalOptions, config.edge_config.products, body, 'products', cb);
                    }
                });
            }
        },
            ...
            if ( config.edge_config.redisBasedConfigCache === true ) { //retrieve info from redis db
                ...
                request.get(opts, function(err, response, body) {
                    ...
                    if(useSynchronizer && !err && response && response.statusCode === 200){
                        loader.saveConfigToRedis(config.edgemicro, globalOptions, config.edge_config.jwt_public_key, body, 'jwt_public_key', cb);
                    }
                });
            }
        },
...
/**
 * Internal method to call the final callback when redis emits error events
 * @param err error object to be passed to callback function.
 * @param cb callback function.
 */
Loader.prototype.callErrorCallback = function (err, cb) {
    try {
        this.redisClientStatus.maxErrorCallbacks--;
            if ( this.redisClientStatus.maxErrorCallbacks > 0 ) { // to handle multiple callbacks 
                cb(err);
            }
        } catch( err ){
            debug('Exception in calling callback ', err);
        }
}
...
Loader.prototype.connectRedisDb = function (config, cb) {
    ...
    if ( this.redisClientStatus.isReady ) {
        return cb();
    }
    ...
    this.redisClient.on('reconnecting',  (options) => {
        debug('redisClient is reconnecting',options);
    });

    this.redisClient.on('end',  () => {
        debug('redisClient connected end');
    });
...
Loader.prototype.saveConfigToRedis = function (config, options, proxy, data, suffixKey, cb) {
    if(!data){
