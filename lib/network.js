...
const _load = function(config, keys, callback) {
    ...
    async.parallel([
        ...
        if (!proxies) {
            console.error('CRITICAL ERROR:','no edge micro proxies found in response');
            return callback(new Error('CRITICAL ERROR:no edge micro proxies found in response'));
        }
        if (proxies.length === 0) {
        ...
        try {
            ...
            console.error('CRITICAL ERROR:', 'error parsing downloaded product list',
                err);
        ...
        if (!productInfo) {
            ...
            return callback(new Error('CRITICAL ERROR: no edge micro products found in response'));
        }
        var products = productInfo && productInfo.apiProduct ? productInfo.apiProduct : [];
        if (!products) {
            console.error('CRITICAL ERROR:', 'no products found in response');
            return callback(new Error('CRITICAL ERROR: no products found in response'));
            // products = [];
        }
        if (products.length === 0) {
            console.error('CRITICAL ERROR', 'no products found in org');
            return callback(new Error('CRITICAL ERROR: no products found in response'));
        }
