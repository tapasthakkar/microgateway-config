...
module.exports.validate = function validate(config) {
  ...
  assert(config.agent, 'config.agent is not defined');
  assert(config.agent.port, 'config.agent.port is not defined');
  assert(typeof config.agent.port === 'number', 'config.agent.port is not a number');
  assert(config.agent.port > 0, 'config.agent.port is invalid (negative)');
  assert(config.agent.port <= 65536, 'config.agent.port is invalid (too large)');
  assert(config.agent.min_uptime, 'config.agent.min_uptime is not defined');
  assert(typeof config.agent.min_uptime === 'number', 'config.agent.min_uptime is not a number');
  assert(config.agent.min_uptime >= 1000, 'config.agent.min_uptime is too small (min 1s)');
  assert(config.agent.spin_sleep_time, 'config.agent.spin_sleep_time is not defined');
  assert(typeof config.agent.spin_sleep_time === 'number', 'config.agent.spin_sleep_time is not a number');
  assert(config.agent.spin_sleep_time >= 1000, 'config.agent.spin_sleep_time is too small (min 1s)');
  assert(config.agent.max_times, 'config.agent.max_times is not defined');
  assert(typeof config.agent.max_times === 'number', 'config.agent.max_times is not a number');
  assert(config.agent.max_times >= 1, 'config.agent.max_times is too small (min 1)');
  ...
  assert(config.edgemicro.logging.dir, 'config.edgemicro.logging.dir is not defined');
  ...
  if (config.edgemicro.plugins) {
    assert(config.edgemicro.plugins.dir, 'config.edgemicro.plugins.dir is not defined');
    assert(typeof config.edgemicro.plugins.dir === 'string', 'config.edgemicro.plugins.dir is not a string');
    assert(config.edgemicro.plugins.sequence, 'config.edgemicro.plugins.sequence is not defined');
