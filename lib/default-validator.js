...
module.exports.validate = function validate(configObject, options={}) {
  ...
  if (config.quotas) {
    ...
    Object.keys(config.quotas).forEach(key => {
      assert( key === 'failOpen' || key === 'useDebugMpId' || key === 'useRedis' || key === 'bufferSize' || key === 'excludeUrls' || key === 'isHTTPStatusTooManyRequestEnabled', 
      'invalid value in config.quotas: ' + key + ', valid values are failOpen, useDebugMpId, useRedis, bufferSize, excludeUrls, isHTTPStatusTooManyRequestEnabled');
      if (  key === 'failOpen') {
      ...
      } else if (key === 'isHTTPStatusTooManyRequestEnabled') {
        assert(typeof config.quotas[key] === 'boolean', 'config.quotas.' + key + ' should be a boolean');
      }
    })  
  }
