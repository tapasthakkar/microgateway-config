...
Apid.prototype.stitch = function(config) {
    ...
    //ar template = Handlebars.compile(JSON.stringify(require('./config-template.js')));
    var template = Handlebars.compile(fs.readFileSync(path.join(__dirname,'config-template.js')).toString())
    var values = {
        deployments: config.map((deployment) => {
            return {
                scope: deployment['scopeuuid'],
                bundles: deployment['bundles'].map(bundle => self.io.loadSync(bundle['uri'].replace('file://', '')))
            }
        })
    }
...
function attach(object, fn) {
    object[fn.name] = fn;
    return object;
}

function fluent(fn) {
    return function fluentize() {
