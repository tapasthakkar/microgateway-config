...
const buildApidEndpoint = require('./build-apid-endpoint');
...
Apid.prototype.get = function (options, callback) {
    ...
    if(options.configFile) {
      try {
        var config = yaml.safeLoad(fs.readFileSync(options.configFile).toString());
        return callback(null, formatConfig(applyDefaults(config)));
      } catch(e) {
        return callback(e);
      }
    }
    ...
    request.get({url: buildApidEndpoint(this.apidEndpoint, '/deployments')}, function (err, response, body) {

        if(err) {
          var newConfig = Object.assign(emptyConfig, self.systemConfig);
          return callback(err, newConfig);
        }
        ...
        self._processResponse(self.apidEndpoint, response, body, (err, newConfig) => {
            ...
            if(err) {
              ...
              newConfig = Object.assign(emptyConfig, self.systemConfig);
              return callback(err, newConfig);
            } else {
              ...
              if(self.apidEndpoint && newConfig['analytics-apid']) {
                newConfig['analytics-apid'].apidEndpoint = self.apidEndpoint;
              }

              //inject the apidEndpoint to verify api key
              if(self.apidEndpoint && newConfig['verify-api-key']) {
                newConfig['verify-api-key'].apidEndpoint = self.apidEndpoint;
              }
              ...
              self._basicValidation(newConfig, (err, valid, invalidPair) => {
                if(!valid) {
                  var errorString = 'Configuration invalid';
                  if(invalidPair.base_path && invalidPair.vhost){
                    errorString = util.format('%s for basepath and vhost (%s %s)', errorString, invalidPair.base_path, invalidPair.vhost);
                  }
                  newConfig = Object.assign(emptyConfig, self.systemConfig);
                  return callback(new Error(errorString), newConfig);  
                } else {
...
Apid.prototype.beginLongPoll = function(clientSocket, pollInterval, retry) {
    ...
    var options = {
        url: buildApidEndpoint(this.apidEndpoint, '/deployments')
    }
    ...
    request.get(options, function (err, response, body) {
        if (err) {
          ...
          setTimeout(()=>{
            console.log("Apid reported 404 waiting 10 seconds, and retrying.");
            self.beginLongPoll(clientSocket, pollInterval, retry);
          }, 10000)
        } else {
            ...
            self.etagValue = response.headers['etag'];
            var parsedBody;
            //This is a strange hack. Apparently I'll get back null as a string.
            if(body !== 'null') {
               parsedBody = JSON.parse(body);
            } else {
              parsedBody = [];
...
Apid.prototype._reportStatus = function(json) {
  ...
  //If we pass in an empty status array. Don't bother with http requests.
  if(!json.length) {
    console.log('No status to report.');
    return;
  }  

  const endpoint = buildApidEndpoint(this.apidEndpoint, '/deployments');
  const opts = {
    url: endpoint,
    json: json,
    headers: {
  ...
  request.put(opts, function(err, resp, body){
    if (err) {
      ...
      if(resp.statusCode === 200) {
        ...
        console.log(util.format("Error sending PUT deployment status back to apid. Status code %s.", resp.statusCode));
      }
    }
  });
}
...
Apid.prototype._processResponse = function (url, response, body, cb) {
    ...
    if (response && response.statusCode !== 200 && response.statusCode !== 404) {
        ...
        //Short circuits everything gives us back raw config
        if(this.printRawConfig){
          return cb(null, this.stitch(body));
        }
...
Apid.prototype._basicValidation = function(config, cb){
  var proxyHostPairs = [];

  var valid = true;
  var invalidPair = {};
  config.proxies.forEach((proxy) => {
    var found = proxyHostPairs.some((pair) => { 
      return pair.base_path == proxy.base_path && 
      pair.vhost == proxy.vhost; 
    });
    if(!found) {
      proxyHostPairs.push({base_path: proxy.base_path, vhost: proxy.vhost});
    } else {
      valid = false;
      invalidPair.base_path = proxy.base_path;
      invalidPair.vhost = proxy.vhost;
      return;
    }
  });

  
  return cb(null, valid, invalidPair);
