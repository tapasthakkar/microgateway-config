...
Apid.prototype.get = function (options, callback) {
    request.get({url: process.env.apid_endpoint}, function (err, response, body) {
        this._processResponse('config', process.env.apid_endpoint, err, response, body, callback);
    }.bind(this));
}
...
const applyDefaults = function(config) {
    console.log(_.merge({}, configDefaults, config));
...
Apid.prototype._processResponse = function (message, url, err, response, body, cb) {
    ...
    if (err) {
        ...
        if (!body.bundles) {
            ...
            var config = this.io.loadSync(body['bundles'].find(bundle => bundle.type == EDGEMICRO_CONFIG_BUNDLE_TYPE)['url']);
